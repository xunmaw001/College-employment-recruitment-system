	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"var(--publicSubColor)","color":"#fff","textAlign":"left","borderRadius":"20rpx 0","borderWidth":"0 10rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.gongsiName" type="text" placeholder="企业名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('gongsi','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.gongsiPhoto?baseUrl+product.gongsiPhoto.split(',')[0]:''">
            </image>
            <view class="list-item-body" style="display: flex;flex-wrap: wrap;">
                <view
                        ...
                        class="title">{{product.gongsiName}}
                </view>
                <view v-if="product.gongsiNewMoney"
                      ...
                      class="title">￥{{product.gongsiNewMoney}}
                </view>
                <view v-else-if="product.gongsiValue"
                      :style='{"padding":"12rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0","borderColor":"red","backgroundColor":"rgba(255, 255, 255, 1)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}'
                      class="title">{{product.gongsiValue}}
                </view>
            </view>
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('gongsi','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>

                <text v-if="isAuth('gongsi','删除')" class="cuIcon-delete"
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let gongsiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'gongsi_types'
		  	});
		  	this.gongsiTypesList = gongsiTypes.data.list;
			gongsiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
            this.categoryList = gongsiTypes.data.list
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.list(`gongsi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('gongsi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.gongsiShenheTime=shijian;
                            await _this.$api.update(`gongsi`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.gongsiName) {
                    searchForm['gongsiName'] = this.searchForm.gongsiName
                }

                let res = await this.$api.list(`gongsi`, searchForm);
				// 如果是第一页数据置空
