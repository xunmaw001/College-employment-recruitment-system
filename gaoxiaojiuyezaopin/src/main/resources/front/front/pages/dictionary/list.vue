<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <view v-if="isAuth('dictionary','新增')" @click="onAddTap()"
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.dictionaryName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('dictionary','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.dicCode" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">字段</text>
                     <text style="width: 70%">
                         <text v-if="item.dicCode != null">{{item.dicCode}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.codeIndex" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">编码</text>
                     <text style="width: 70%">{{item.codeIndex}}</text>
                 </view>
                 <view v-show="item.beizhu" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">备注</text>
                     <text style="width: 70%">
                         <text v-if="item.beizhu != null">{{item.beizhu}}</text>
                         <text v-else></text>
                     </text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('dictionary','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('dictionary','审核') && item.dictionaryYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.dictionaryYesnoTypes=shenheTypes;
                            data.dictionaryShenheTime=shijian;
                            await _this.$api.update(`dictionary`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
