	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.zhaopinLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('zhaopinLiuyan', this.zhaopinLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.zhaopinLiuyanPhoto ? this.detail.zhaopinLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhaopinLiuyanYuyue', {
									yonghuId: _this.user.id,
									zhaopinLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/zhaopinLiuyanYuyue/list');
							}
						}
					});
				},
