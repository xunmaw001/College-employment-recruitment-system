	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('toudi', this.toudiId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.toudiPhoto ? this.detail.toudiPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('toudiYuyue', {
									yonghuId: _this.user.id,
									toudiId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/toudiYuyue/list');
							}
						}
					});
				},
