	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.zhaopinCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('zhaopinCollection', this.zhaopinCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.zhaopinCollectionPhoto ? this.detail.zhaopinCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhaopinCollectionYuyue', {
									yonghuId: _this.user.id,
									zhaopinCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/zhaopinCollectionYuyue/list');
							}
						}
					});
				},
