<template>
	<view>
        ...
        <view class="username">
			...
			<text v-if="forum.gongsiId != null">
				 发布人：{{forum.gongsiName}}（企业）
			</text>
			...
			<view class="uni-article__comment-box" v-for="comment in commentData" :key="comment.id" >
				<view class="article-meta">
					<view class="article-header-image">
                        ...
                        <image v-if="comment.gongsiId != null" class="image" :src="baseUrl+comment.gongsiPhoto" mode="widthFix"></image>
					...
					<view class="article-header-content">
						<view class="article-header-title">
							...
							<text v-if="comment.gongsiId != null">{{comment.gongsiName}}</text>
