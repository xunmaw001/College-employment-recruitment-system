	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"var(--publicSubColor)","color":"#fff","textAlign":"left","borderRadius":"20rpx 0","borderWidth":"0 10rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.zhaopinName" type="text" placeholder="招聘信息名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('zhaopin','新增')" @click="onAddTap()"
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='zhaopinTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in zhaopinTypesList" :key="index" :class="zhaopinTypes===item.codeIndex?'tabActive':''"
                                  @tap="zhaopinTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item"
            ...
            <view style="display: flex;justify-content: space-between;">
                <text v-if="isAuth('zhaopin','修改')" class="cuIcon-edit"
                      @click.stop="onUpdateTap(product.id)">修改
                </text>

                <text v-if="isAuth('zhaopin','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 招聘岗位字段类别搜索
			zhaopinTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.zhaopinTypes != 0) {
                    params.zhaopinTypes = this.zhaopinTypes
                }
                let _this = this
                let res = await this.$api.list(`zhaopin`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('zhaopin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.zhaopinShenheTime=shijian;
                            await _this.$api.update(`zhaopin`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.list(`zhaopin`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.zhaopinContent!=null){
                        item.zhaopinContent = item.zhaopinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
