			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.zhaopinName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.zhaopinContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
				...
				<view class="bottom-content cu-bar bg-white tabbar border shop">
					<view style="width: 100%;margin: 0 auto;display: flex;align-items: center;justify-content: space-around;">
						...
						<!-- 没理由
						<view @tap="" style="display: flex; width: 80%;color: #FFFFFF;">
							<view :style="[{backgroundImage:'linear-gradient(120deg,'+ btnColor[0] +','+ btnColor[1] +')'}]" style="width: 95%;height: 80rpx;line-height: 80rpx;text-align: center;border-radius: 40rpx;">
								我要报名
							</view>
						</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.zhaopinId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getzhaopinCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        zhaopinId: this.zhaopinId,
                        yonghuId: this.user.yonghuId,
						zhaopinLiuyanPingfenNumber: this.scoreData,
                        zhaopinLiuyanText: this.commentData,
                    }
                    await this.$api.save(`zhaopinLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('zhaopin', this.zhaopinId);
                    this.detail = res.data
					this.detail.zhaopinContent = this.detail.zhaopinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.zhaopinPhoto ? this.detail.zhaopinPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getzhaopinCollection() {
					let params = {
						page: 1,
						limit: 1,
						zhaopinId: this.zhaopinId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`zhaopinCollection`, params);
					if (res.data.list.length == 1) {
						let zhaopinCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('zhaopinCollection', JSON.stringify([zhaopinCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getzhaopinCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('zhaopinCollection', {
									yonghuId: _this.user.id,
									zhaopinId: _this.detail.id,
									zhaopinCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('zhaopinLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						zhaopinId: this.zhaopinId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('zhaopin', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.save('toudi', {
									zhaopinId: _this.detail.id,
								});
								_this.$utils.msg('投递成功');
								_this.$utils.jump('/pages/toudi/list');
							}
						}
					});
				},
