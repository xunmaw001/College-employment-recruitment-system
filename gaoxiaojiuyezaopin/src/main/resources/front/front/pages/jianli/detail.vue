	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('jianli', this.jianliId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.jianliPhoto ? this.detail.jianliPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('jianliYuyue', {
									yonghuId: _this.user.id,
									jianliId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/jianliYuyue/list');
							}
						}
					});
				},
